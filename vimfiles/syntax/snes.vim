...
syntax keyword snesDirective db dw dl org base incsrc incbin include includefrom
  \ includeonce lorom hirom exhirom exlorom sa1rom fullsa1rom sfxrom norom skip
  \ warnpc pushpc pullpc print table pushtable pulltable freespace freedata
  \ freecode align cleaned static autoclean freespacebyte ram noram prot warn
  \ error assert check title bankcross warnings disable enable arch math pri
  \ round on off namespace nested bank dpbase optimize dp always none address
...
syntax keyword impreciseInstruction
  \ ADC AND BIT CMP CPX CPY EOR LDA LDX LDY ORA SBC STA STX STY STZ TRB TSB
...
syntax keyword preciseInstruction
  \ ADC.b ADC.w ADC.l AND.b AND.w AND.l ASL.b ASL.w BCC BCS BEQ BIT.b BIT.w
  \ BMI BNE BPL BRA BRK BRL BVC BVS CLC CLD CLI CLV CMP.b CMP.w CMP.l COP
  \ CPX.b CPX.w CPY.b CPY.w DEC.b DEC.w DEX DEY EOR.b EOR.w EOR.l INC.b INC.w
  \ INX INY JMP JMP.w JML JSR JSL LDA.b LDA.w LDA.l LDX.b LDX.w LDY.b LDY.w
  \ LSR.b LSR.w MVN MVP NOP ORA.b ORA.w ORA.l PEA PEI PER PHA PHB PHD PHK
  \ PHP PHX PHY PLA PLB PLD PLP PLX PLY REP ROL.b ROL.w ROR.b ROR.w RTI RTL RTS
  \ SBC.b SBC.w SBC.l SEC SED SEI SEP STA.b STA.w STA.l STP STX.b STX.w STY.b
  \ STY.w STZ.b STZ.w TAX TAY TCD TCS TCD TRB.b TRB.w TSB.b TSB.w TSC TSX TXA
  \ TXS TXY TYA TYX WAI WDM XBA XCE

syntax keyword errorInstruction
  \ ASL.l BIT.l CPX.l CPY.l DEC.l INC.l LDX.l LDY.l LSR.l ROL.l ROR.l STX.l STY.l
  \ STZ.l TRB.l TSB.l
